...
RB <- all_fantasy %>% filter(all_fantasy$FantPos == "RB") %>% group_by(Player)
QB <- all_fantasy %>% filter(all_fantasy$FantPos == "QB") %>% group_by(Player)
WR <- all_fantasy %>% filter(all_fantasy$FantPos == "WR") %>% group_by(Player)
TE <- all_fantasy %>% filter(all_fantasy$FantPos == "TE") %>% group_by(Player)
...
age_age2_model <- lmer(Fantasy_PPR ~ Age + Age2 + Rushing_Att + Receiving_Tgt + (1|Player), data=RB)

predict(my_model, data.frame(Player = "Jamaal Charles", Age = 27, Age2 = 729))
